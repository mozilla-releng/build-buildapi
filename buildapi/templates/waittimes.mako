...
% if wt['pool'] == 'buildpool':
    <b>buildpool</b> |
% else:
    <a href="/waittimes/buildpool?${request.query_string}">buildpool</a> |
% endif
% if wt['pool'] == 'trybuildpool':
    <b>trybuildpool</b>
% else:
    <a href="/waittimes/trybuildpool?${request.query_string}">trybuildpool</a>
% endif
</p><p>
<label for="starttime">Jobs submitted between</label>
<input type="text" id="starttime" name="starttime" ref="${wt['starttime']}"/>
<label for="endtime">and</label>
<input type="text" id="endtime" name="endtime" ref="${wt['endtime']}"/>
<input type="button" id="timeIntervalButton" value="Go!" onclick="updateWindowLocation();"/>
</p>
<br/>
</div>
<div>
<div>Wait time report for <b>${wt['pool']}</b> for jobs submitted between <b>${time.ctime(wt['starttime'])}</b> 
and <b>${time.ctime(wt['endtime'])}</b></div>
...
<div class="clear"></div>
<br/>Generated at ${time.ctime()}. All times are Mountain View, CA (US/Pacific).

<script type="text/javascript">
$(document).ready(function() {
    $(".wt-overall").dataTable({
        "bFilter": false,
        "bSort": false,
        "bAutoWidth": false,
        "bPaginate": false,
